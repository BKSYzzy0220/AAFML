...
class Meta(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        self.trigger_optimizer = torch.optim.Adam([self.t], lr=0.1)
        ...
        self.trigger_optimizer_mini = torch.optim.Adam([self.t_mini], lr=0.1)  # 使用Adam优化器更新触发模式
        #self.t = torch.nn.Parameter(torch.ones((1, args.c_, args.h, args.w), ), requires_grad=True)
        ...
        for client_id in selected_clients:
            if client_id in self.adv_task_indices:
                grad, loss, correct = self.malicious_client_update(
                    ...
                    x_qry[client_id], y_qry[client_id], m)
            else:
    ...
    def malicious_client_update(self, x_spt, y_spt, x_qry, y_qry, mask):
        ...
        x_spt_mod = x_spt_mod - mask * self.t
        x_qry_mod = x_qry_mod - mask * self.t
        y_spt[:] = 0
        y_qry[:] = 0
